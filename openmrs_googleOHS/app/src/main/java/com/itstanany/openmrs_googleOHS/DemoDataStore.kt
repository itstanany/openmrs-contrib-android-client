...
private val Context.dataStorage: DataStore<Preferences> by preferencesDataStore(name = "demo_app_storage")
...
class DemoDataStore(private val context: Context) {
  ...
  suspend fun saveUserIdToken(token: String) {
    context.dataStorage.edit { pref -> pref[stringPreferencesKey("USER_ID")] = token }
  }

  suspend fun getUserIdToken(): String? {
    return context.dataStorage.data.first()[stringPreferencesKey("USER_ID")]
  }
}